...
const { robot, payload } = getState();
const intoPayload = canMoveForward(robot.row, robot.col, robot.direction);
assert(intoPayload.ok === false, "payload blocks robot movement");
assert(intoPayload.occupant === "payload", "payload occupant reported");
...
assert(ontoProp.ok === false, "cannot release payload onto solid prop");

payload.carried = true;
payload.row = null;
payload.col = null;
assert(
  getCellOccupant(level01.payload.row, level01.payload.col) === null,
  "carried payload frees its former cell",
);
